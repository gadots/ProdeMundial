-- Migration 014: Fix check_prediction_window blocking points calculation
--
-- Problem: check_prediction_window() fires on BEFORE INSERT OR UPDATE on
-- predictions and raises an error whenever now() >= match kickoff time.
-- This blocked calculate_match_points() from writing points_earned, making
-- points impossible to save for any finished match.
--
-- Fix: allow updates that only touch points_earned / updated_at (i.e. the
-- scoring system writing results). Block only changes to the actual
-- prediction values (home_goals, away_goals, multiplier, penalty_winner).
--
-- Also re-applies the calculate_match_points fix from migration 013
-- (which rolled back due to this same error) and re-runs the backfill.

-- ─── 1. Fix check_prediction_window ──────────────────────────────────────────
...
begin
  -- Allow the scoring system to write points_earned without restriction.
  -- Detect this case: prediction values unchanged, only points_earned/updated_at differ.
  if TG_OP = 'UPDATE'
     and new.home_goals      = old.home_goals
     and new.away_goals      = old.away_goals
     and new.multiplier      = old.multiplier
     and coalesce(new.penalty_winner, '') = coalesce(old.penalty_winner, '')
  then
    return new;
  end if;

  select date into match_date from public.matches where id = new.match_id;
  if match_date is not null and now() >= match_date then
...
-- ─── 2. Fix calculate_match_points (rec.id bug from migration 009) ───────────
